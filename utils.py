...
class ClickType:
    ...
    def input_file(writable=False):
        return click.Path(
            exists=True, file_okay=True, dir_okay=False, writable=writable, readable=True, resolve_path=True
        )

    @staticmethod
    def input_dir(writable=False):
        return click.Path(
            exists=True, file_okay=False, dir_okay=True, writable=writable, readable=True, resolve_path=True
        )

    @staticmethod
    def output_file():
        return click.Path(
            exists=False, file_okay=True, dir_okay=False, writable=True, readable=True, resolve_path=True
        )

    @staticmethod
    def output_dir():
        return click.Path(
            exists=False, file_okay=False, dir_okay=True, writable=True, readable=True, resolve_path=True
        )
...
def train_test_sp_to_utt(audio, is_test):
    ...
    for speaker_id, utterances in audio.speakers_to_utterances.items():
        ...
        sp_to_utt[speaker_id] = (
            utterances_files[train_test_sep:] if is_test else utterances_files[:train_test_sep]
        )
    return sp_to_utt
